...
| `hermes-tweet` | X/Twitter 搜索、资料查询、回复读取、监控和受控动作 | X/Twitter |
...
hermes skills install https://raw.githubusercontent.com/keihang/hermes-article-skills/main/hermes-tweet/SKILL.md
...
cp -r hermes-article-skills/hermes-tweet ~/.hermes/skills/
```
...
# hermes-tweet 使用
# 在 Hermes 运行环境中设置 XQUIK_API_KEY，不要发到聊天里
export HERMES_TWEET_ENABLE_ACTIONS=false              # 需要发帖/私信等动作时再显式改为 true
```
...
- 需要搜索 X/Twitter、读取回复、查询用户资料、监控账号或执行受控动作 → 使用 `hermes-tweet`
